                ...
                <div class="row">
                    <div class="col-xl-12 col-md-12 col-sm-12 col-12">
                        <a class="btn btn-primary"
                           href="{{route('sub_cat.create.new',$cat_id)}}">{{ __('messages.add') }}</a>
                    </div>
                </div>
            </div>
                            ...
                            <tr id="id_{{ $row->id }}">
                                ...
                                @if(Auth::user()->update_data)
                                    <td class="text-center blue-color"><a
                                            href="{{ route( 'sub_cat.edit', $row->id ) }}"><i
                                                class="far fa-edit"></i></a></td>
                                @endif
                                @if(Auth::user()->delete_data)
                                    <td class="text-center blue-color">
                                        @if (count($row->ViewSubCategories) > 0 || count($row->products) > 0)
                                        {{ __('messages.categories_has_plans') }}
                                        @else
                                        <a
                                            onclick="return confirm('{{ __('messages.are_you_sure') }}');"
                                            href="{{ route('sub_cat.delete', $row->id) }}"><i
